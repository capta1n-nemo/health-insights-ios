...
/// as movement.
public struct PeerStandingInsight: InsightModel {
    public let id: InsightID = .peerStanding
    public let title = "Where You Stand"
    public init() {}

    public var candidateMetrics: [MetricType] {
        [.restingHeartRate, .heartRateVariabilityRMSSD, .vo2Max]
    }
    public var requirements: [GroundingRequirement] {
        [.init(kind: .dateOfBirth, isMandatory: true,
               rationale: "Every one of these norms is age-banded — without it there is nobody to compare you to."),
         .init(kind: .biologicalSex, isMandatory: true,
               rationale: "The reference distributions are published separately by sex.")]
    }
    ...
    public func evaluate(samples: [HealthMetricSample], profile: UserHealthProfile,
        ...
        guard let output = PeerStandingModel.evaluate(samples: samples, profile: profile,
                                                      now: now) else {
            return InsightResult(
                id: id, title: title, primaryValue: nil,
                headline: unmet.isEmpty ? "Needs a reading" : "Add your details",
                score: nil, confidence: .low,
                explanation: "Your numbers compared against published figures for people your age and sex — the context every app leaves out. Needs your age and sex, plus a recent resting heart rate, HRV or VO₂max.",
                drivers: [], unmetRequirements: unmet)
        }

        let drivers = output.standings
            .sorted { $0.percentile > $1.percentile }
            .map { standing in
                InsightDriver(
                    text: String(format: "%@ %@ — %@ for your age and sex",
                                 standing.metric.displayName,
                                 MetricValueFormatter.detailedString(standing.value, standing.metric),
                                 standing.phrase),
                    // The weak ones are what a person would want to look at.
                    isNotable: standing.percentile < 40)
            }

        let headline = output.best.map { "\($0.metric.displayName): \($0.phrase)" }
            ?? "\(Int(output.overall.rounded()))th centile"

        return InsightResult(
            id: id, title: title, primaryValue: output.overall,
            headline: headline, score: output.overall,
            confidence: output.standings.count >= 3 ? .high : .moderate,
            explanation: "Across \(output.standings.count) measure\(output.standings.count == 1 ? "" : "s") you sit around the \(Int(output.overall.rounded()))th centile for people your age and sex. These are approximations to published population figures, not a lookup into a real distribution — and a centile describes where you sit, not whether anything is wrong.",
            driverLines: drivers.filter { $0.isNotable == true }
                + drivers.filter { $0.isNotable != true },
            unmetRequirements: unmet,
            contributors: output.standings.map { standing in
                MetricContribution(metric: standing.metric, higherIsBetter: nil, weight: 0,
                                   detail: standing.phrase)
            })
    }
}
