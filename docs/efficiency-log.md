...
| 30 | 2026-08-07 | **1** | **0** | 0 | 0 | 1912 → 1912 (docs/tooling only) | **One open-item list, read by one script that fails loudly.** `docs/backlog.md` absorbs `progress.md`'s roadmap table and `activeContext.md`'s two hand tables — 215 rows, 151 open, nothing deleted. **`scripts/backlog.sh`** replaces `roadmap-table.sh` and `unbuilt-asks.sh`, and its contract **hard-errors** on a line that starts a row but does not parse, and on a section carrying table rows but no backlog rows — the exact shape §B7 and §B9–§B19 hid in. Both failure paths tested by reproducing the original defect. **Every row carries a `tier`** — `mech`/`build`/`hard`/`ultra`/`design` — naming the model and effort it needs, so work batches by complexity and a session changes model once and then grinds; `--next` prints it. `handover-check.sh` now gates on `backlog.md`, and `handover.md`, `CLAUDE.md` and `session-start` all name one list. The two retired scripts exit non-zero pointing at the replacement rather than being deleted | **Cheapest session in the log on every measured column — and the measured columns are the least interesting thing about it.** 0 waste / 1 push, zero red CI, installed first time. **What it actually bought: twenty corrections to the audit of record.** Fifteen rows were telling sessions that shipped work was unbuilt — calendar persistence, both calendar cards, the fitness sections, all five "cards with no bespoke section" — and `D17`'s two named causes were *both wrong* (the seed export predates the gait promotion commit by one day, so 27,248 walking-speed rows sit in `unmodelled`). ⚠️ **The sting is a logged repeat at instance four, and it was inside the countermeasure**: `unbuilt-asks.sh`, built in session 28 to stop the three-session "where is my work?" repeat, reported **2** open asks this morning against **63**. It parsed two hard-coded headings. **Both of that ledger row's automations failed by returning an under-count rather than an error** — which is the whole lesson, and why the replacement refuses to run rather than skipping a row it cannot read. Also rescued: `AC1`, the 706-line calendar-drift patch, which the docs said was probably gone and which survives in the abandoned iCloud checkout's scratchpad on a reboot clock |

### Session 30 notes — the named counts

**Red CI (0).** One push, green first time, deploy `installed`. Docs and shell
only; `swift test` 1,912 pass, unchanged.

**Rework (0 shipped).** Three defects were caught *by the new parser before the
commit* rather than after it, which is the tool working rather than rework: two
id collisions (§B9's `C1`/`C2` against §B6's calendar-brief `C1`–`C10`, renamed
`B9-1`/`B9-2`) and six sections carrying tables with no rows. Four of those six
were legitimate — §A's answered decisions are deliberately prose so they cannot
be re-asked — which is why the contract gained a **declared** opt-out
(`<!-- backlog: prose-only — why -->`) rather than a silent exemption.

**Re-derivations (0).** The ground-truth fan-out re-read code the docs already
covered, which normally *is* the expensive mistake `CLAUDE.md` names. It was not
one here: the task was to establish whether three lists agreed, and they did not
— twenty times. Re-verification was the deliverable, not a substitute for
reading.

**The repeat, and why it is worth the space.** The ledger row *"the reader
having to ask where a requested feature is"* is at **four** instances, and the
fourth was the automation built for the third. Session 28 shipped
`unbuilt-asks.sh`; it parsed `^### B2 —` and `^### B5 —` and rows shaped
`| <digits> |`. Every brief the reader wrote after that — §B7 on 2026-08-06,
§B9–§B19 on 2026-08-07 — used `##` headings and `H1`/`C1`/`S1` ids, and was
invisible to it. **The script did not break; it under-reported, silently, and an
under-report reads exactly like good news.**

That is the finding to carry: *a parser that silently returns nothing reads as
"nothing outstanding"*. `backlog.sh` therefore refuses to run when a row does
not parse, and refuses when a section has table rows and no backlog rows. Both
paths were tested by re-creating the original failure — appending a brief in the
old table shape and watching the gate go red.

**What was deliberately not done.** The waves and tiers on 215 rows are a
judgement call made once, in bulk. They will be wrong in places. They are
cheap to correct — one field on one line — and `--next` surfaces the wrong ones
quickly, which is the argument for shipping them rather than agonising. Say so
when one is wrong rather than working around it.
