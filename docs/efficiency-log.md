...
| **The reader having to ask where a requested feature is** | **4** | ✅ **re-automated 2026-08-07 — the first automation failed the same way it was built to prevent.** `scripts/unbuilt-asks.sh` (2026-08-06) parsed `^### B2 —`/`^### B5 —` and rows shaped `\| <digits> \|`. The reader's 2026-08-07 brief (§B9–§B19, 22 rows) and holidays brief (§B7, H1–H7) are `##`-level with `H1`/`C1`/`S1` ids, so they were **structurally invisible to it** — on 2026-08-07 it printed **two** items while 63 of the reader's own asks were open. Previously: sessions 25, 27, 28, each time the session doing good work on what it found interesting and leaving the reader's *named* asks unstarted. **The instance count is now four and the automation is the fourth.** ⬜→✅ Replaced by `scripts/backlog.sh --asks`, over one list, with a contract that **hard-errors rather than skipping** — a section carrying table rows but no backlog rows fails the build, because that is exactly the shape those briefs hid in. ⚠️ **The generalisable lesson, and the reason this row is worth re-reading: a parser that silently returns nothing reads as "nothing outstanding".** Both of this row's failed automations returned an under-count, not an error. Fail loud. |
| **A multi-branch merge splicing additive edits into code that does not parse** | **2** | ⬜ **open, and the top unautomated row.** Eleven branches each added a `DataDomain` case, an `InputKind` case, an export key and a `Schema` entry; an automatic keep-both resolution welded two `case` lists together, closed the `Schema` array twice and left two `syncWarning` defaults. Earlier the same session, four files kept conflict markers because the marker check ran over the file list `git` reported *before* resolution rather than over the tree. **The fix is a command, not care**: after any multi-branch merge, `grep -rl '^<<<<<<<'` over the whole tree, and where a merged array carries a rule (`DataStore`'s *"a @Model not listed here silently never persists"*), enumerate the set from source and compare. Worth a `verify.sh` check — conflict markers in a tracked file should fail the gate outright |
| **A merge reverting a fix the same session shipped** | **1** | ✅ **automated by accident, and it is the strongest argument in this table for linting a rule rather than writing it.** All eleven branches predated `D62`'s fix, so the merge restored `await refreshTagApplicability()` in front of the sync's completion marker — re-shipping an outage the reader had reported hours earlier. **No test can see that defect**; 2,669 were green. The `verify.sh` lint written alongside the fix caught it within the hour. **Every fix whose defect is invisible to tests should land with its lint in the same commit** |
| **Diagnosing a flaky gate as a real failure** | **1** | ⬜ open — see `D63`. The app-target suite failed three times under twelve-agent load, differently each run, and I diagnosed it wrong twice before nearly pushing past the gate on a `git stash` comparison that was contaminated by container state. **The gate was right all three times.** A red gate that is usually environmental trains a session to push through a real one; the fix is for the failure to *say* it is a resource problem |
| **Three open-item lists disagreeing with each other** | **1 (found), years accumulating** | ✅ **automated 2026-08-07** — `docs/progress.md`'s generated roadmap table, `docs/backlog.md` and two hand tables in `docs/activeContext.md` all recorded open work. A ground-truth pass across all three found they **disagreed in twenty places**, with **fifteen rows describing work that had already shipped** — a session planning from them would have scheduled rebuilds of shipped code. Now one list, one script, and `handover-check.sh` requires `backlog.md` rather than `progress.md`, so there is no ritual edit that keeps a second list alive. |
| **A hand-written status document becoming a fourth list** | **1** | ✅ **automated 2026-08-08, hours after it was created.** The reader asked for a delivered/partial/not-started breakdown; the answer was `docs/status-2026-08-07.md` — 534 hand-written lines, correct that morning, and structurally the same object `backlog.sh` had just retired three of. `scripts/status.sh` now generates `docs/status.md`: the feature half from `backlog.md`, the research half from each research document's own `<!-- status: … -->` line. **The generalisable form: when the reader asks for a view of state, build the view as a projection, never as a copy.** A copy is a promise to keep two things in step, and this repo has already proved three times that it cannot keep that promise |
| **A research document on disk with no machine-readable verdict** | **1 (19 instances at once)** | ✅ **automated 2026-08-08** — 15 of 19 research/design documents carried no parseable status, so "we researched that" and "that research reached a conclusion" were indistinguishable from outside the file. **Two of the 19 were *refuted* designs** (`energy-design-2026-08-07`, `stress-design-2026-08-07`) that a later session could have built from in good faith — and one of them contained a fabricated statistic. `status.sh` now hard-errors on a research doc without a status line, so the verdict is written when the memory of it still exists |
| **A session transcript or health export sitting untracked and unignored in a public repo** | **1** | ✅ **automated 2026-08-08.** `last_session.md`/`.rtf` — 357 KB each, RTF, a full transcript — sat at the repo root while this session ran `git add -A` three times. Nothing but staging luck stood between a transcript full of real readings and a pushed commit, in a public repo holding one person's health data. `.gitignore` retires the instance; `verify.sh` **content-sniffs the git index** to retire the class, because a filename is a guess about what is inside a file and the leak is the content. Canaried |
| **`grep -q` under `pipefail` failing on an early match** | **1 (3 sites)** | ✅ **automated by removal, 2026-08-08.** Red CI on `a3d70d6` with the identical tree green locally: `printf '%s' "$body" \| grep -qE …` — `grep -q` exits at its first match and closes the pipe, `printf` takes SIGPIPE, and `pipefail` **fails the pipeline because the match succeeded**. macOS delivers the body in one write and never trips it; Linux chunks it. Converted to herestrings in `verify.sh` and, before it could ever fire, in `pre-push-gate.sh`. **This is the "guard whose own premise is false" row in shell form** — the check assumed the pipeline's exit code was grep's, and `pipefail` makes that false in exactly the success case. Zero instances remain in `scripts/` |
| **A backlog row still describing shipped work as unbuilt** | **2** | ⚠️ partly — `I9` read *"Mesh renders; no capture exists"* for a week after the guided capture shipped in `1dc9789`; a session planning from it would have rebuilt a shipped feature, which is the consolidation's own failure mode surviving *inside* the consolidated list. Found by an agent doing the adjacent work, not by any check. Earlier the same session, `P22` and `R24` were marked ✅ with bodies still saying "is not started". ✅ **automated 2026-08-08, in the same handover that wrote this row** — `scripts/lint-stale-rows.py`, run by `verify.sh`: a ✅ row whose *body* still says "is not started" / "no capture exists" / "is unbuilt" fails the gate. The title is exempt (a defect's own name is often "X does not exist"), quoted spans are exempt (this repo quotes superseded claims on purpose), and a row that needs the phrase declares `<!-- stale-ok: why -->`. Canaried both ways. **This row is worth re-reading for the sequence, not the fix**: it was written asserting no mechanical check was possible, and writing that sentence down is what made the check obvious — the glyph and the prose are both free text, but they are both free text *in the same file, five lines apart*. ⚠️ **The step-12 sweep is still the only thing that catches the other direction** — a ⬜ row describing work that has shipped, which is what `I9` actually was |
| **A wave of agents closing rows and reporting no new findings** | **0 (watched for)** | ⬜ open, recorded as a *tell* rather than an instance. This session's final wave closed 12 rows and produced **15 new ones**, and the open count went 40 → 55. That is the healthy shape: an agent that finishes real work and finds nothing adjacent has either done something trivial or lost what it learnt. Worth asking of any wave whose report is all `closed` and no `newRows` |
| **A hand-written summary of a generated list going stale** | **3** | ⬜→✅ `activeContext.md`'s "what the reader has asked for and has NOT got" table listed the Fitness sections, sound exposure and breathing disturbance as unstarted when all three had shipped; §0's build-order table listed rows 6 and 7 the same way. Both are commands now (`backlog.sh --asks`, `--next`) rather than tables. **The rule that generalises: if a list is long enough to want a summary, the summary must be generated** — it is stale the first time somebody ticks a box. |
...
| **Parallel branches colliding on a monotonically-numbered list** | **3** (chartStyleIndex 74 twice; the DataTabSortingTests homeless set; backlog D40/D41 minted twice) | ⬜ **unautomated — top candidate.** Git takes both sides and neither is wrong, so there is no merge resolution. A generator, the way `roadmap-table.sh` already does the roadmap |
| **Hand-rescuing work from a dead agent's worktree** | **4** this session alone | ⬜ unautomated. Every agent that could not commit (D30) or died mid-run left work only a human-driven `git -C <worktree>` could save. A `scripts/rescue-worktrees.sh` that commits every dirty agent worktree to its own branch would retire it |
| **A repo in iCloud Drive breaking the build** | **3** — codesigning (extended attributes), 766 MB syncing, and finally **TCC revoking read access to a live session** | ✅ **fixed 2026-08-07 — moved to `~/health-insights-ios`.** The first two were worked around; the third could not be |
...
### Session 31 notes — 2026-08-08

**Red CI: 2**, both mine, both from one bug.
- `a3d70d6` — the exhaustive-switch lint I added the previous day used
  `printf '%s' "$body" | grep -qE`. Under `pipefail` that pipeline fails when grep
  matches *early*, because grep closes the pipe and `printf` takes SIGPIPE. It
  reported `environmentalSoundDose` missing from a file containing it three times.
  Green on macOS, red on Linux, identical tree.
- `45c450b` — the merge commit that carried the same lint into `main`.

**Rework: 1.** The lint fix itself (`f59e17f`), plus a pre-emptive conversion of
the two remaining instances in `pre-push-gate.sh` (`6b1e7d4`) that had not fired
because they only run on macOS.

**Re-derivations: 0.** Named honestly: I re-read `.claude/commands/handover.md`
and `scripts/backlog.sh`'s parsing rather than working from memory, which is the
intended cost, not waste.

**What the eight-agent wave actually cost and bought.** 1.39M subagent tokens,
744 tool calls, 41 minutes wall clock. Twelve rows closed. **Fifteen new rows
written** from things agents found while doing something else — and the honest
accounting is that those fifteen are worth more than several of the twelve,
because `J6` (Readiness has six weights and no source) and `J8` (seven surfaces
spend the same nightly triad, uncounted) are the same class of finding as the
signal audit that prompted this whole stream, and neither had a row before.

⚠️ **One agent of the eight returned no report.** It had written
`docs/writeback-scope-2026-08-08.md` — 214 lines, complete — and never committed
it; the workflow's own diagnostics said *"subagent completed without calling
StructuredOutput"*. It was recovered by looking in the worktree rather than
trusting the empty result. **Carry this: a null report is not a null result, and
the worktree is the place to check before writing the work off.**

⚠️ **Two workflows and one agent from earlier in the session were reported
`stopped` with no completion record** — they were running when the previous
process exited. Their transcripts survive on disk. Same rule: check the worktree,
then the journal, before assuming nothing landed.

### Session 28 notes — 2026-08-06

⚠️ **Red CI (1).** `77847b2` — `import FoundationModels` without a
`#if canImport` guard. CI's runner SDK has no such module, so the app target
failed to compile; the local gate could not see it because this Mac's SDK does.
**The file it belongs beside, `FoundationModelSummarizer`, has guarded that exact
import since it was written** — so this is the "a new reader of X violating a
documented trap of X on first use" ledger row, at two.

*Retired as a category*: `verify.sh` now fails on a bare
`import FoundationModels`. ⚠️ **And the first version of that lint was wrong** —
it asked whether the file mentioned `canImport` *anywhere*, and this file guards
two call sites with it, so the bare import at the top sailed straight through. A
canary caught it. **A lint nobody canaries is a lint nobody has tested**, which
is the second time that has been written down here.

Every other push was waited on with `ci-status.sh` before the next.
...
| 28 | 2026-08-06 | **19 (21 commits)** | **1 (named below)** | 0 shipped (4 reversals caught before commit — see below) | 1 (named below) | 1628 → 1716 | **`BiologicalAgeModel`** — a biological age with no fitted parameter anywhere: every marker inverted through a published age norm, combined by inverse-variance weighting (σ = population spread ÷ the curve's own slope), so a flat marker demotes itself and gait speed is worth nothing at forty and a great deal at eighty from one table and no special case; **chronological age deliberately excluded**, which costs an honest ±11 years the card leads with rather than the tight number every commercial version prints; **`MicronutrientEstimate`** — `MicronutrientTargets` had been dead code while the Nutrition card made sex and DOB *mandatory because of it*, so an ask whose stated reason never happened is now paid for; **`BloodPressureEstimator.statedUncertainty`** — one error bar, the widest of fit-spread / measured-miss / the ISO cuff floor, which both retires Q2's two-± contradiction and is what makes ungating the estimator safe; **the balance-web rule** — *a card whose best answer is "nothing found" cannot share an axis with cards that grade a level*, now in `belongsOnBalanceWeb` and asserted in both directions by `BalanceWebTests`; **the feedback control ungated**, so the cards most likely to be wrong are no longer the ones the reader cannot tell you are wrong; **`bespokeSection` made exhaustive** — `default: EmptyView()` deleted, so a new card cannot ship without a stated decision about its own picture, and five sections written in one pass; **`ScoreBlend` stops discarding `term.score`** — one dropped field was why no card could answer "why is my score low" for the life of the app, and `ScoreDecomposition`'s counterfactual is closed-form arithmetic rather than a re-run, refusing by name on every weighting that is not linear in its parts; **`OAuthTokens` is no longer `Codable`** — a token cannot be a stored property of any `Encodable` type, which turns the export's no-tokens promise from a convention into a compile error; **`SymptomRadarModel.dayCounters`** — three counters that were computed and thrown away on every evaluation, so the radar can finally state its own flag rate and coverage; **the cycle tab's first slice** — a working period tracker whose defining rule is that a cycle length is a *range with its spread*, never an average, with no `averageLength` property for a view to reach for; **the age section takes every source and our own** — it read one instrument through `VitalReader.reading`, which is right for a vital and exactly wrong on the screen whose subject is that instruments disagree; **`CalendarIntegration`** — the only blocker on two cards the reader had asked for by name, with a test that fails if `CalendarEvent` grows a title or a location, so the privacy shape is a compile-time property rather than a convention; **`CalendarEventClassifier`** — the six axes the reader asked for, with the rules answering everything that is arithmetic or a field being present and the model asked only about the two genuinely interpretive ones, `refined` refusing to let it overrule a fact, and the reader's correction stored *beside* the guess so accuracy is measurable at all | **Zero red CI across three pushes, and the cheapest column is not the interesting one.** ⚠️ **The interesting number is six: six defects found by opening the app, none of which any of 1,658 tests could fail on**, because each is a claim about what a number *means* — a biological age of 22 whose own explanatory section read "heart-rate variability carries 95% of it"; three of five markers deleted by a single read window; a clamped marker voting *hardest* because its slope was read out in the steepest tail; excluding clamped markers then deleting two of the reader's five; a dial reading 30 beside a headline of "close to your years"; and **"Mind 80" drawn green beside Fitness 33**, which reads as *your mind is fine and your body is not* — the one claim that card exists to refuse, arriving through the chart rather than the copy. **The cost sits in a column this table does not have**: roughly ten build-launch-navigate-screenshot cycles to read numbers the model already held, which is now the ledger's top unautomated row and the roadmap's top item. **The structural finding worth more than any of it**: these norm curves are medians, and for several markers the whole span of adult ageing is narrower than the spread between people at one age — so clamping deletes exactly the readers whose markers are furthest from typical. ⚠️ **A seventh defect landed after the first handover and it is a logged repeat**: a new section printed "All four" on a card running on three signals, and mental health named all four behaviours including one it had no data for — the ledger's "hard-coded count going stale" row, at 4+ sessions, except inside a sentence, where it is worse because the sentence is a claim about what was looked at. Found on a screenshot again, not by a test. **And the session's single best decision cost one question**: a scouting fleet found the cycle tab blocked on something unanswerable from the repo — the app is structurally single-user and the tab is for someone else's body — so it was asked rather than assumed. The answer (she installs on her own phone) made the cheapest of three possible builds the correct one; assuming either of the others would have meant a profile dimension through every baseline and every card. ⚠️ **And the session's real failure is one the table cannot show, for the third session running: the reader had to ask *"where is the travel card, and all the other cards I asked for?"*** — eleven commits in, with travel drain and work impact still unstarted and both blocked on a calendar integration they had explicitly instructed. Sessions 25 and 27 record the same shape. The fix is not more care: it is to read `docs/backlog.md` §B2 — *cards asked for and not built* — at the START of a session |
| 29 | 2026-08-06/07 | **21 (64 commits)** | **2 (named below)** | 4 (named below) | 0 shipped | 1716 → 1912 | **The reader used the app on their own phone all day and reported five defects no test could see** — the sync hang (`recompute()` measured at 2.36 s over 380k samples on the `@MainActor`, from 33 call sites), Work impact scoring the body and not the calendar, the one-shot review row, and two copy defects. **`use-the-phone`** — iPhone Mirroring plus computer-use closes the largest class of unverifiable claim in this project, and the review-loop fix was seen before *and* after on real data. **`CoverageGate`**, **a description on every data entry** (non-optional, so the compiler enforces it), **every derived figure a data source across all 18 cards**, and **`docs/illness-detection-evidence-2026-08-07.md`**, which constrains a feature the reader specced the same day. ⚠️ **The repo left iCloud Drive** after it revoked read access mid-session | **The worst waste in the log, and it was environmental.** ~10 agents died on API stalls or the filesystem loss; three had written real work that needed hand-rescue, one lost everything, and the perf audit re-derived a finding already fixed hours earlier. 2 red CI in 21 pushes = 0.10, against 28's 0.05 — but the red is not the story. **The compounding lesson: an agent that only needs the web survived; an agent that needs the filesystem did not.** The two best deliverables of the day had no filesystem dependency at all |

| 30 | 2026-08-07/08 | **28 (123 commits)** | **5** | 3 (named below) | 0 | 1912 → **2669** + 40 app-target | **One list, and a script that fails loudly.** `docs/backlog.md` absorbs `progress.md`'s roadmap table and `activeContext.md`'s two hand tables; **`scripts/backlog.sh`** replaces `roadmap-table.sh` and `unbuilt-asks.sh` and **hard-errors** on a row it cannot parse or a section carrying tables but no rows. Every row gains a **tier** naming the model it needs, at the reader's request. **The app target has tests for the first time** (`D5`, 40 of them). **`waitingOn()`** beside `invitingInput` — the other half of D46, for gates the reader cannot act on. **`isWorthShowing` is unconditionally true** on the reader's ruling. Three lints that each caught a real defect within the hour: the on-device-model-in-the-refresh-path check, the lazy-cache-in-the-export check, and the rule-4/rule-5/hard-coded-count trio | **Cheapest waste ratio of any long session, and the ratio is not the story.** 8 waste / 28 pushes = **0.29** against a 0.56 baseline, across 123 commits and ~90 rows closed by five agent waves. ⚠️ **What it actually bought: twenty corrections to the audit of record** — fifteen rows were telling sessions that shipped work was unbuilt. ⚠️ **And five live defects the reader found on their own phone**, all fixed and installed: the sync never completing, a card vanishing, the export carrying no score history, an exclusivity trap aborting the app, and the `+` menu. **The sting is that three of those five were caused by this session's own work**, each by a *correct* fix with an unconsidered second effect |
| 31 | 2026-08-08 | **4** | **2 (both mine, named below)** | 1 (the lint I shipped red) | 0 | 2669 → **2756** + 56 app-target | **`scripts/status.sh` — the reader's status question answered as a *projection* instead of a copy**, killing the fourth list the day it was born, and **hard-erroring on a research document with no verdict** (15 of 19 had none, two of them refuted designs); **a tracked transcript or export fails the gate**, content-sniffed from the git index, after 357 KB of RTF session transcript sat at the repo root unignored through three `git add -A` calls in a public health-data repo; **`grep -q` under `pipefail` removed from every script** — the SIGPIPE shape that fails *because* the match succeeded, converted in `pre-push-gate.sh` before it could ever fire; **the Thread Performance Checker reaches real sessions** via a runtime-relative dylib that survives an Xcode update by construction; **`backlog.sh --json`**, so nothing has to re-implement the row contract; **`lint-stale-rows.py`** — a ✅ row whose prose still calls the work unbuilt fails the gate, written in the same handover that recorded the category as unautomatable | **Two red CI in four pushes — the worst ratio in this log — and both were mine, in the same lint.** 0.50 against a 0.56 baseline and session 30's 0.29. ⚠️ **The honest reading is that the ratio is the wrong measure for a four-push session**: one bug caused both, it was a Linux-only shell trap in a check I had written the previous day, and the second red was the *merge* of the wave that carried it. **What it bought: the tell was one line nobody reads** — `printf: write error: Broken pipe` — and reading it closed a category across three call sites rather than fixing one lint. **The session's real value is in a column this table does not have**: 12 rows closed by 8 agents, and **15 new rows written from what those agents found while doing something else**, so the open count *rose* 40 → 55. A wave that reports only closures has lost what it learnt. ⚠️ **And one stale row is the whole consolidation's failure mode surviving inside the consolidated list**: `I9` said "no capture exists" for a week after the capture shipped |

### Session 30 notes — the named counts

**Red CI (5), and only one was a real failure.**
- `e689fb9` — **genuine.** The allow-entry lint I added asserted a premise false
  anywhere but this Mac: it derived the repo root from `git rev-parse` and
  required a matching `settings.json` entry, so CI's `/home/runner/work/…` root
  failed. This repo's own logged class — *a guard reporting a failure whose own
  premise is false* — pointed at itself. Fixed in `c411fef`, canaried in three
  environments.
- `25a276b`, `cbb6aa6`, `49ee10b`, `d63350b` — **consistent with the documented
  `cancel-in-progress` pattern** (session 27's row): each was followed by another
  push before its run finished, and **none wrote a `refs/ci/errors` blob**, so
  none was a compile failure. ⚠️ I could not fully distinguish a superseded run
  from a lint failure after the fact, so all five are counted. The lesson is
  session 27's, unlearnt: **a push that is not waited on is a push whose result
  nobody knows.**

**Rework (3).**
1. The conflict-marker sweep ran over `git`'s pre-resolution file list rather
   than the tree, so four files kept markers and were committed. Found by the
   compiler.
2. The `D63` diagnosis, twice — a container-size theory (wrong, 12 MB) and a
   dedicated-simulator theory (wrong, would not boot under load), before the
   real answer was load flakiness.
3. Over-marking 27 backlog rows closed from my own cluster list when the agents
   had closed 15. Reverted and re-marked from the reports. ⚠️ **This is the
   failure the whole session existed to fix, committed by me, in the file built
   to prevent it.**

**Re-derivations (0).** The ground-truth fan-out re-read code the docs already
covered, which is normally the expensive mistake `CLAUDE.md` names. It was not
one here: the task was to establish whether three lists agreed, and they did not
— twenty times.

**What was automated so it cannot recur.**
- `backlog.sh` refuses to run on an unreadable row, retiring the
  silently-returns-nothing class that both previous ask-trackers died of.
- A lint per invisible-to-tests defect: `D62`'s (an on-device model call awaited
  in the refresh path) **caught the same bug being reverted by a merge, the same
  day**; `D60`'s catches the export reading a lazy view cache.
- `SyntheticSeed` is exhaustive, so a new metric or card cannot ship with an
  empty simulator — and it earned itself at a merge, refusing to compile until
  two agents' new types declared what a seeded simulator shows.
- `simulator.sh` gets a per-worktree slot and prints the installed build's
  timestamp, after an agent screenshotted another agent's build and believed it.

**The honest note.** Three of the five reader-reported defects were caused by
this session's own work, each by a fix that was correct in itself: stopping a
card lying about the calendar hid the card; making the export honest about
sharing exposed the lazy cache; adding a section triggered an exclusivity trap.
**The pattern is a second-order effect nobody looked for** — and the only thing
that caught any of them quickly was a lint written in the same commit as the fix.

### Session 29 notes — the named counts

**Red CI (2).**
- `5ecb246` (the review row) — **local gate Clean, CI red, and the very next
  commit green with no code change between them.** Almost certainly the
  `LaunchParticleField` scaling test again: its *sampling bias* was fixed this
  session (equal work on both sides of the ratio) and that fix holds locally
  under load average 33, but CI's shared runner is a different machine. ⚠️ **The
  bias fix did not fully retire the flake on CI hardware** — it is the fourth
  appearance of this assertion and the next one should consider deleting the
  timing test rather than repairing it a fifth time.
- `77847b2` (the calendar brief) — inherited from the previous chat session's
  tail, inside this handover's commit range.

**Rework (4).**
- `feb4672` → the record-then-wipe fix. The derived-series commit put the
  recording loop *above* `invalidateDerivedCaches()`, so every sync recorded 18
  results and erased them a line later. Mine, same session.
- `077fd6c` → untracking ten worktree gitlinks that `git add -A` had committed
  to a public repo two commits earlier. Mine.
- `2f4c608` → renumbering backlog D40/D41, minted twice by two branches.
- The `AppModel` merge conflict where the cycle branch's side would have
  re-introduced the record-then-wipe bug — caught, but it is rework caused by
  parallel branches.

**Re-derivations (0 shipped, 1 near-miss worth naming).** The performance-audit
agent independently re-derived the `recompute()` main-actor finding that had
been fixed hours earlier. It was not in the docs when the agent started, so it
is not a docs-re-derivation — but ~255k subagent tokens produced a #1 finding
already closed, and **the audit's caller list was the genuinely new part**. The
lesson is about *sequencing an audit against live work*, not about the docs.

**The honest headline.** Twenty-one pushes and two red is a good ratio. It is
not what happened. Roughly ten agents died — API stalls, then the filesystem —
and the recovery work (hand-rescuing three worktrees, rebuilding `CoverageGate`
from scratch, re-launching four agents that had written nothing) was most of the
afternoon. **None of it is visible in the columns this table has**, which is the
second time the log has flattered a session whose real cost was elsewhere.
