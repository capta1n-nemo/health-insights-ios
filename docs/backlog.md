...
| 7 | Sound exposure (#33 ◐ — the two dose metrics shipped, the card remains) · sleep apnoea (#30, ⚠️ **identifiers now requested — count the rows before building**) · ~~radar accuracy~~ ✅ `44fb94e` | §B5 #30, #33, #36 |
...
| 30 | **Sleep-apnoea card** | ✅ **SHIPPED as a Sleep section** — breathing disturbance index, trended, never asserted.  Asserting or screening for apnoea is FDA-clearance territory. Trending the index inside Sleep is fine; a card whose *name* implies a condition is not | **BUILD IT** |
...
| 33 ◐ | **Total sound exposure** | Environmental audio exists on 14 of the last 90 days — summing it with headphones would invent the quiet hours | **BUILD IT.** ◐ **The dose metrics shipped 2026-08-06**: `environmentalSoundDose` and `headphoneSoundDose`, each the day's equal-energy LEQ in dBA over the hours its own sensor could hear (`SoundDoseModel`, on the ingest path, `MetricSource.calculated`, Data tab ▸ Hearing) — never summed into one figure, and the denominator is the measured time precisely so no quiet hour is ever invented. Nothing scores them yet. ⬜ **Still to come: the card** — #22's shape, headphone dose as the number, environmental beside it **with its coverage stated** |
| 34 | **Physical-effort intensity** | ✅ **SHIPPED `5ed4ac7`** — `EffortIntensityModel`, WHO substitution, feeds the Fitness score (`fitness-v2`).  81,252 rows looks dense and is a trap: 13 of the last 90 days. A z-score over a series that exists one day in seven is not a z-score | **BUILD IT — as a Fitness section**, which is what the original note already recommended. ⚠️ **And the reader added a requirement the refusal never considered: the effort score feeds the overall Fitness score.** That is new work, not a relocation |
| 35 | **Steps / distance / flights** | ✅ **SHIPPED `5ed4ac7`** — three metrics promoted, Fitness sections.  Real, but Fitness sections rather than a card | **BUILD IT — as Fitness sections.** Reader agrees with the placement |
...
| # | Piece | State |
|---|---|---|
| R1 | **The review row stops being one-shot.** Today choosing one axis commits and dismisses. It must let the reader change **as many axes as they want** — context, formality, whether it was a meeting, travel, presence — and only then confirm. Nothing leaves the list on a single tap || ✅ **SHIPPED** — draft layer, every axis editable, Change un-confirms |
| R2 | **Confirm is reversible, and confirmed rows stay.** An un-confirm, and a confirmed row remains visible with its state shown rather than vanishing. ⚠️ The reader's own emphasis: *"when i do confirm it should stay in the list, not disappear!"* || ✅ **SHIPPED** — draft layer, every axis editable, Change un-confirms |
| R3 | **The correction record keeps all three layers**: what the model guessed, what the reader corrected it to, **and the artifact it judged** (title, location, attendees, duration — the whole event). C4 already keeps guess and correction apart, which is the hard half; the artifact snapshot is what is missing, and it is what makes a correction *trainable* rather than merely counted | ✅ **SHIPPED** `ce9f76f` |
| R4 | **All three reach the export and the improvement section** | ⬜ |
| R5 | **Two-tier sharing replaces the binary toggle**, both tiers on by default, at the reader's explicit instruction. **Full** — the artifact plus the before/after categories. **Metadata only** — the correction shape without the content ("the BP estimate ran 13 mmHg above the cuff", "a work guess was corrected to personal") | ✅ **SHIPPED** `ce9f76f` |
| R6 | **Every claim that nothing leaves the device is rewritten to be true.** Settings ▸ Data & model improvement, `TelemetryEvent`'s doc comment, `DiagnosticsLog`'s class comment, the export's own copy. ⚠️ **A stale privacy promise is worse than no promise** — it is the one kind of copy a reader is entitled to rely on | ✅ **SHIPPED** `ce9f76f` |
...
| D23 | ✅ **`recompute()` recorded the derived series and wiped them a line later** — the record loop sat above `invalidateDerivedCaches()`, which resets the store, so every sync recorded 18 results and erased them immediately. Found on the simulator: "Refresh complete — 18 insights" in diagnostics while the Data tab's Generated-insights row stayed absent | Fixed same session, order swapped. The general shape: **a cache-invalidation call at the end of a function silently owns everything recorded above it** |
| D24 | ✅ **`InsightResult.appending(driverLines:)` silently dropped `subheadline`** — the same rebuild-field-by-field shape that lost `invitesInput` on 2026-08-05. Fixed with `derivedOutputs` forwarding in `713ae0b`; the warning is now written at the site | There is still no compiler check for the next field added to `InsightResult` |
| D25 | **The component tier of Generated insights will be thin until the ~31 contribution sites fill `componentScore`/`z`** — the fields are optional and defaulted, most models still leave them nil, so "do the sub-scores too" currently only covers the cards that fill them. Worth a sweep: each nil is a card whose decomposition the deep dive also cannot show | Same fields feed backlog #38's decomposition, so one sweep pays twice |
| D26 | **`DiagnosticsLog` cannot be read from outside the app** — it is in-memory for Settings ▸ Troubleshooting only, with no os_log mirror, so `simulator.sh logs` cannot see it and a session debugging the app has to click through the UI to read its own diagnostic line. Cost two build-launch cycles this session | A DEBUG-only os_log mirror would make `simulator.sh logs` sufficient |
| D27 | **Raw-field rows render metadata as measurements** (UI/UX, Data tab): "How it was measured — 0", "Device model ID — 16", "Measurement categ… — 1" read as nonsense readings; long names ("Environmental Sound Reduct…", "Daily readiness temperature de…") truncate with no way to read the full name on the row | The session-27 "32.00 years" class, still visible in the Not-yet-sorted group |
| D28 | **Oura's stress fields sit in "Not yet sorted"** — `Daily readiness score 58`, `Day summary "stressful"`, `Stress high 9900`, `Recovery high 0`, `Daily resilience level "limited"` render unsorted and unexplained at the bottom of the Data tab, while N1 (stress research) names exactly these fields as the free comparison | Sorting them into a labelled group is cheap and makes N1's raw material visible |
| D29 | **The Data tab has grown very long** (UI/UX): with the raw catalogue loaded it is dozens of screens of scroll; the search field is the only fast path and a reader has to know a field's name to use it | Consider collapsed-by-default raw groups, or an index |
| D30 | ⚠️ **`scripts/bash-workdir-hook.sh` makes any worktree-isolated agent unable to commit** — it rewrites every shell call to `cd "$CLAUDE_PROJECT_DIR"`, which is the *main* repo, so a worktree agent's git calls are refused by the isolation guard and its relative paths hit the wrong tree. Two agents lost their commit this session; one lost it entirely and its doc had to be rescued by hand. Hook fixed on 2026-08-06 to read the payload's `cwd` — **the fix only takes effect for agents spawned after it lands on `main`** | Found live, `docs/setup-review-2026-08-06.md` finding 1 |
| D31 | **`.claude/settings.json:24` allows the hook's `cd` prefix only for the Linux path** — the Mac and worktree prefixes have no allow entry. One-line fix, left for a main session because settings.json was outside the review agent's permit | Same review |
| D32 | **Concurrent worktree agents collide on shared scratchpad filenames** — one agent's git-helper script was twice rewritten by other agents to point at *their* worktrees with the safety check removed. Every agent that commits must self-check `git rev-parse --show-toplevel` immediately before staging | Observed 2026-08-06 |
| D33 | **`ContributionSummary.bodyMeasurements` and `.screenTime` are tested nowhere** — `ContributionSummaryTests.swift:223`'s hand-built list has 7 of 9 route factories, so three "across every route" sweeps silently exclude two. The CardVisibilityTests closed-set lesson, live again | `docs/test-audit-2026-08-06.md` #1 |
| D34 | **Whoop never got the 2026-08-04 timezone fix** — `WhoopResponseParser.parseSleep:89` takes no calendar and defaults `Calendar.current`; its only sleep test never asserts onset, so the bug class that hit Oura is untested here | Test audit #2 |
| D35 | **The export has no decode round-trip anywhere** — `HealthDataExportTests` is all `json.contains(...)`, and one test titled "must survive the round trip" never decodes. This is exactly the 2026-08-05 import shape-mismatch defect's shape | Test audit #5 |
...
| D41 | ✅ **Work impact scores the calendar as well as the body, 2026-08-06.** The reader overruled the weight-0 answer in the same session that shipped it: *"I want all inputs to carry at least some weight, thats the entire point. If i have had 10 meetings in a day, how would that not leave me impacted and drained?"* The card's number now blends **exposure** (the gap between busy and quiet working days, and the heaviest single day — both as multiples of the reader's own typical working day, never an invented band) with **response** (the same RHR/HRV/sleep comparison as before). Response carries 65% at full contrast and 30% at none, so a body difference measured across a flat calendar is discounted rather than blamed on work; the calendar carries the rest, and no calendar row is at weight 0 any more. Four quadrants — carrying / absorbing / unexplained / quiet — are named in the headline and the first driver line. `work-impact-v2`; **travel drain deliberately stayed v1** | `WorkImpactModel.Quadrant` carries the design; new `ScoreBlend.blend(metrics:factors:metricShare:)` is the shared machinery. **Open, and named at the site:** both exposure facets are ratios, so they are scale invariant — a diary that is heavy *every* day reads light. The real fix is scoring this fortnight against the reader's own trailing year once `workExposure` has a history, not a constant |
...
| D43 | ⚠️ **The app announces a discovery for data it then throws away.** `AppModel.observeArrivals` records what *arrived* and runs **before** `partitionedVitals()` sanitises, so a reading outside its metric's `plausibleRange` is announced as "New since you last looked" and then dropped — the reader's single vitamin A entry does exactly this. The Data-tab row now appears regardless (fixed 2026-08-06), so the symptom is covered, but **the ledger is still recording arrivals that never became data**, which will mislead any future reader of it. Decide: observe after sanitising, or record the drop alongside the sighting so the row can say "arrived, but outside the plausible range" | Found while fixing the discovered-row bug |
| D44 | ✅ **`load-real-export.sh` was half a no-op on any container that had run the app.** `loadCachedOther` prefers `synced_other.hirc`, and the loader deleted only `synced_samples.hisc` — so 177,000 raw rows were written to a file nothing read, and the app kept showing the previous run's catalogue. Cost a full simulator cycle to find | Fixed same session; both compact caches are deleted now |
| D45 | **Sleep Duration's trend chip and its detail describe different things, side by side.** On Work impact the row reads "Holding steady · higher is better · 22% of this score" while its own detail says "about the same on your busier working days — 6.7 against 7.4 — which is 0.2 SD worse". The chip is the metric's *trend*; the detail is the *busy-versus-quiet contrast*. Both true, adjacent, and they read as a contradiction — which erodes trust in the numbers around them. Either the chip belongs elsewhere on a contrast card, or it has to name which question it answers | Seen on the reader's own phone, 2026-08-06 |
| D46 | **Every "not enough yet" gate is invisible.** The reader's point, and the calendar review section already does it right — *"It needs 10 before that figure means anything"*. Nothing else does: 3 cycles before a phase prediction, 3 worn days before an effort dose, 5 days per week before a norm contribution, 2 contributors before a replayed score point. A reader who hits one sees an absence and cannot tell "no data" from "not enough yet" — and the second is a reason to keep going | Reader, 2026-08-06: *"it needs to be mentioned appropriately for transparency, so users know why things are - or are not showing in the app"* |
