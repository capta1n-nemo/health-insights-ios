...
Superseded narrative, moved out of `docs/activeContext.md` so it stops being
read at every session start. **Nothing here is current.** It is kept because the
reasoning behind a decision outlives the decision, and this repo's rule is to
move rather than delete.

For what is open now, run `./scripts/backlog.sh`. For what shipped and what
found it, read `docs/progress.md`. For where things stand, read
